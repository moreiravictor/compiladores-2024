...
package br.edu.ufabc.isilanguage.compiler.parser;
	...
	public ATN getATN() { return _ATN; }
		...
		public void verificaCompatibilidade(ArrayList<Integer> tipos) {
			...
			for (int tipo: tipos) {
				if (tipoEsq != tipo) {
					...
					throw new IsiException(errorMsg);
				}
			}
			tipos.removeAll(tipos);
		}
		...
		public void verificaAttrib(String id) {
			if (symbolTable.exists(id) && symbolTable.get(id) == null)
				throw new IsiException(String.format("\"%s\" has not been initialized.", id));
		}

		public void checkInitialized(String id) {
	        if(!symbolTable.checkInitialized(id))
	            throw new IsiException("Symbol "+id+" not initialized");
	    }
	    ...
	    public void showWarningsUnusedVariables() {
	        for(Symbol s: symbolTable.getNotUsedSymbols())
	            System.out.println("Warning: Variable <" + s.getName() + "> declared but not used");
	    }
		...
		public ArrayList<String> getWarnings() {
	        ...
	        for(Symbol s: symbolTable.getNotUsedSymbols())
	            warnings.add(String.format("Warning: Variable <%s> declared but not used", s.getName()));
	        return warnings;
	    }
			...
			match(ID);
				                      	 ...
				                      	 if (!symbolTable.exists(_varName))
				                      	    symbolTable.add(symbol);
				                      	 	 ...
				                      	 	 throw new IsiException("Symbol "+_varName+" already declared");
